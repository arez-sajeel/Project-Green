...
# Added `db["usage_logs"].delete_many({})` to the startup
# script to prevent duplicate data on every server restart.
...
from backend.routers import optimisation
import sys
import typing
...
async def simulate_real_time_feed(logs: List[HistoricalUsageLog]):
    ...
    if redis_client is None:
        print(
            "NFR-P2 Simulation: Redis client not available. Task stopping.",
            file=sys.stderr,
        )
        return
    ...
    try:
        for log_entry in simulated_stream:
            ...
            payload = (
                f"{log_entry.timestamp.isoformat()}|{log_entry.kwh_consumption:.2f}"
            )
    ...
    try:
        ...
        if logs:
            try:
                ...
                # --- START OF SPRINT 4.3 FIX (Duplicate Data) ---
                # First, delete all existing documents to prevent
                # duplicates on every server restart.
                print(
                    "Task 2.2: Clearing old usage data from 'usage_logs' collection...",
                    file=sys.stdout,
                )
                delete_result = await db["usage_logs"].delete_many({})
                print(
                    f"Task 2.2: Deleted {delete_result.deleted_count} old documents.",
                    file=sys.stdout,
                )
                # --- END OF SPRINT 4.3 FIX (Duplicate Data) ---
                ...
                print(
                    f"Error (Task 2.2): Failed during bulk persistence: {e}",
                    file=sys.stderr,
                )
        ...
        print(
            f"FATAL: Failed to load or simulate data on startup: {e}", file=sys.stderr
        )
...
app = FastAPI(
    ...
    version="0.4.3",  # Incremented version for Task 4.3
...
# --- START OF MODIFICATION (SPRINT 4.3) ---
# Include the new optimisation router
app.include_router(
    optimisation.router, prefix="/optimisation", tags=["Optimisation"]
)
# Note: The endpoint will be POST /optimisation/run-scenario
# --- END OF MODIFICATION (SPRINT 4.3) ---
...
async def read_root():
    ...
    """
    return {"message": "Welcome to the Green Energy Optimizer API (v0.4.3)"}