...
class OptimisationEngine:
    ...
    def run_scenario_prediction(self, request: ShiftValidationRequest) -> OptimisationReport:
    # --- END OF MODIFICATION (SPRINT 4.2) ---
        """
    ...
    def simulate_load_subtraction(self, usage_curve: pd.DataFrame, device: Device, time: datetime) -> pd.DataFrame:
        """
    ...
    def structure_report_output(self) -> OptimisationReport:
        ...
        try:
            ...
            logging.info(f"Successfully structured report with {len(curve_models)} data points.")
            return report
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Internal error structuring final report: {e}"
            )
    # --- END OF MODIFICATION (SPRINT 4.2) ---
    ...
    # --- START OF MODIFICATION (SPRINT 4.2) ---
    def _transform_curve_to_models(self, usage_curve: pd.DataFrame) -> List[UsageDataPoint]:
        """
        ...
        if usage_curve is None or usage_curve.empty:
            logging.warning("_transform_curve_to_models received an empty or None curve.")
            return []
